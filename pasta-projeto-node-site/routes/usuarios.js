...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    if (usuario == undefined || senha_senha == undefined || nome == undefined || email == undefined) {
    ...
    return banco.sql.query(`select count(*) as contagem from projetoBrandao where login = '${usuario}'`);
  }).then(consulta => {
    ...
    res.status(500).send(erro);
	  ...
	  if (cadastro_valido) {		  
		...
		banco.sql.query(`insert into usuario (nome, login, senha) values ('${nome}','${usuario}','${nova_senha}','${email}')`).then(function() {
